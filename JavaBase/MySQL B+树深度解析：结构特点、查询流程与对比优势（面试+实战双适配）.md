...
### B+树中聚簇索引的非叶子节点存放数据，一个节点可以放置一行数据，如果某一个字段为text这种大文本字段可能只会在节点中存放指针，实际值会在溢出页中。

InnoDB 的基本存储单位是**数据页（Page）**，默认大小为 16KB。当一行数据的长度超过数据页的可容纳空间（或页内剩余空间不足）时，InnoDB 会将行中**过长的字段内容**迁移到独立的 “溢出页” 中，而原数据页仅保留该字段的**指针（地址）** 和少量前缀数据，以此保证数据页的高效管理。

#### 对查询性能的影响

- **读性能**：访问包含溢出页的行时，需要先读原数据页，再根据指针读溢出页，多一次磁盘 IO，性能略降（尤其是大量访问大字段时）；

- **写性能**：插入 / 更新大字段时，需要同时操作原数据页和溢出页，写入耗时增加；

- 优化建议：

  1. 避免在高频查询的表中存储超大`TEXT/BLOB`字段，可拆分到独立表；
  2. 对`VARCHAR`字段合理设置长度（如无需 65535 则设为实际需要的长度）；
  3. 定期优化表（`OPTIMIZE TABLE`），减少碎片化导致的溢出页。

  

## 四、溢出页与 B + 树的关联（补充）

溢出页**不属于 B + 树的索引结构**，而是独立的物理页：

- B + 树的叶子节点（聚簇索引）存储的是行记录的 “核心部分”（含溢出页指针）；
- 溢出页仅用于存放超长字段内容，通过指针与原行记录关联，不参与索引排序 / 查找。
