...
![img](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c41b0d2dde577711ec76.png)

1. 缓存

   这是Redis应用最广泛地方，基本所有的Web应用都会使用Redis作为缓存，来 度。
   ![img](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c43b0d2dde57771218ec.png)
...
## [3.Redis 有哪些数据结构？](http://www.susan.net.cn/interview/12-Redis.html#_3-redis-有哪些数据结构)

![img](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c44f0d2dde5777123440.png)
Redis有五种基本数据结构。
...
作者：后端实战笔记
链接：https://www.zhihu.com/question/58185372/answer/2050528817106704374
来源：知乎
...
4. C语⾔实现，优化过的数据结构，基于⼏种基础的数据结构，redis做了⼤量的优化，性能极⾼
   ![Redis使用IO多路复用和自身事件模型](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c46d0d2dde5777126108.png)
...
![多路复用模型](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c47c0d2dde577712770a.png)
...
![Redis6.0多线程](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c4a00d2dde577712aa5f.png)
...
![Redis持久化两种方式](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c4ad0d2dde577712bde9.png)
...
![save和bgsave](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c4bc0d2dde577712d1ee.png)
...
![AOF工作流程](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c4ca0d2dde577712e5bb.png)流程如下：
...
## [11.Redis的数据恢复？](http://www.susan.net.cn/interview/12-Redis.html#_11-redis的数据恢复)

当Redis发生了故障，可以从RDB或者AOF中恢复数据。

恢复的过程也很简单，把RDB或者AOF文件拷贝到Redis的数据目录下，如果使用AOF恢复，配置文件开启AOF，然后启动redis-server即可。
![Redis启动加载数据](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c4da0d2dde577712fca9.png)
...
![混合持久化](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c4e50d2dde5777130f9e.png)
...
![Redis主从复制简图](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c4f00d2dde57771320fb.png)
...
![主从数据同步方式](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c52e0d2dde5777137621.png)
...
![部分复制](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5470d2dde577713998c.png)
...
## [17.主从复制存在哪些问题呢？](http://www.susan.net.cn/interview/12-Redis.html#_17-主从复制存在哪些问题呢)
...
![Redis Sentinel](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5530d2dde577713ae35.png)
...
![Redis Sentinel工作流程](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c55f0d2dde577713c1c0.png)
...
- **主观下线和客观下线**
  ...
  ![主观下线和客观下线](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5750d2dde577713e2cd.png)
...
- **故障转移**
  ...
  ![故障转移](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5810d2dde577713f28d.png)
...
## [20.领导者Sentinel节点选举了解吗？](http://www.susan.net.cn/interview/12-Redis.html#_20-领导者sentinel节点选举了解吗)

Redis使用了Raft算法实 现领导者选举，大致流程如下：
![领导者Sentinel节点选举](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c58c0d2dde5777140332.png)
...
## [21.新的主节点是怎样被挑选出来的？](http://www.susan.net.cn/interview/12-Redis.html#_21-新的主节点是怎样被挑选出来的)

选出新的主节点，大概分为这么几步：
![新的主节点](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5970d2dde5777141552.png)
...
## [22.Redis 集群了解吗？](http://www.susan.net.cn/interview/12-Redis.html#_22-redis-集群了解吗)

前面说到了主从存在高可用和分布式的问题，哨兵解决了高可用的问题，而集群就是终极方案，一举解决高可用和分布式问题。
![Redis 集群示意图](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5a10d2dde57771425ef.png)
...
## [23.集群中数据如何分区？](http://www.susan.net.cn/interview/12-Redis.html#_23-集群中数据如何分区)

分布式的存储中，要把数据集按照分区规则映射到多个节点，常见的数据分区规则三种：
![分布式数据分区](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5ac0d2dde57771436fa.png)

#### [方案一：节点取余分区](http://www.susan.net.cn/interview/12-Redis.html#方案一-节点取余分区)
...
![节点取余分区](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5b80d2dde5777144ae4.png)

#### [方案二：一致性哈希分区](http://www.susan.net.cn/interview/12-Redis.html#方案二-一致性哈希分区)
...
![一致性哈希分区](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5c10d2dde57771458ef.png)
...
#### [方案三：虚拟槽分区](http://www.susan.net.cn/interview/12-Redis.html#方案三-虚拟槽分区)

这个方案 一致性哈希分区的基础上，引入了 **虚拟节点** 的概念。Redis 集群使用的便是该方案，其中的虚拟节点称为 **槽（slot）**。槽是介于数据和实际节点之间的虚拟概念，每个实际节点包含一定数量的槽，每个槽包含哈希值在一定范围内的数据。
![虚拟槽分配](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5ce0d2dde5777146dc2.png)
...
## [24.能说说Redis集群的原理吗？](http://www.susan.net.cn/interview/12-Redis.html#_24-能说说redis集群的原理吗)

Redis集群通过数据分区来实现数据的分布式存储，通过自动故障转移实现高可用。

#### [集群创建](http://www.susan.net.cn/interview/12-Redis.html#集群创建)
...
![分配槽](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5f10d2dde577714a533.png)

#### [故障转移](http://www.susan.net.cn/interview/12-Redis.html#故障转移)
...
![主观下线](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c5fc0d2dde577714b8b0.png)
当某个节点判断另一个节点主观下线后，相应的节点状态会跟随消息在集群内传播。通过Gossip消息传播，集群内节点不断收集到故障节点的下线报告。当 半数以上持有槽的主节点都标记某个节点是主观下线时。触发客观下线流程。
![主观下线和客观下线](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c6080d2dde577714cbf8.png)

**故障恢复**

故障节点变为客观下线后，如果下线节点是持有槽的主节点则需要在它 的从节点中选出一个替换它，从而保证集群的高可用。

![故障恢复流程](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c6130d2dde577714e70e.png)
...
4. 选举投票
   ...
   ![选举投票](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c61e0d2dde5777150103.png)
5. 替换主节点
...
## [25.说说集群的伸缩？](http://www.susan.net.cn/interview/12-Redis.html#_25-说说集群的伸缩)

Redis集群提供了灵活的节点扩容和收缩方案，可以在不影响集群对外服务的情况下，为集群添加节点进行扩容也可以下线部分节点进行缩容。
![集群的伸缩](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c62b0d2dde5777152174.png)其实，集群扩容和缩容的关键点，就在于槽和节点的对应关系，扩容和缩容就是将一部分`槽`和`数据`迁移给新节点。

例如下面一个集群，每个节点对应若干个槽，每个槽对应一定的数据，如果希望加入1个节点希望实现集群扩容时，需要通过相关命令把一部分槽和内容迁移给新节点。
![扩容实例](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c6350d2dde57771536b7.png)缩容也是类似，先把槽和数据迁移到其它节点，再把对应的节点下线。

# 缓存设计

26.什么是缓存击穿、缓存穿透、缓存雪崩？

#### [缓存击穿](http://www.susan.net.cn/interview/12-Redis.html#缓存击穿)

一个并发访问量比较大的key在某个时间过期，导致所有的请求直接打在DB上。

![缓存击穿](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c6420d2dde5777154b29.png)
解决⽅案：

1. 加锁更新，⽐如请求查询A，发现缓存中没有，对A这个key加锁，同时去数据库查询数据，写⼊缓存，再返回给⽤户，这样后⾯的请求就可以从缓存中拿到数据了。
   ![加锁更新](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c64d0d2dde5777155fbf.png)
2. 将过期时间组合写在value中，通过异步的⽅式不断的刷新过期时间，防⽌此类现象。

#### [缓存穿透](http://www.susan.net.cn/interview/12-Redis.html#缓存穿透)

缓存穿透指的查询缓存和数据库中都不存在的数据，这样每次请求直接打到数据库，就好像缓存不存在一样。

![缓存穿透](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c65b0d2dde57771579ef.png)
...
![缓存空值/默认值](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c6700d2dde577715a6bf.png)
...
![布隆过滤器](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c67e0d2dde577715c399.png)
两种解决方案的对比：
![缓存空对象核布隆过滤器方案对比](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c6880d2dde577715d8b5.png)

#### [缓存雪崩](http://www.susan.net.cn/interview/12-Redis.html#缓存雪崩)

某⼀时刻发⽣⼤规模的缓存失效的情况，例如缓存服务宕机、大量key在同一时间过期，这样的后果就是⼤量的请求进来直接打到DB上，可能导致整个系统的崩溃，称为雪崩。

![缓存雪崩](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c6930d2dde577715ed72.png)
...
## [29.如何保证本地缓存和分布式缓存的一致？](http://www.susan.net.cn/interview/12-Redis.html#_29-如何保证本地缓存和分布式缓存的一致)
...
![延时双删](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c6fd0d2dde577716ab8d.png)
...
![本地缓存/分布式缓存保持一致](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7080d2dde577716c353.png)
...
## [30.怎么处理热key？](http://www.susan.net.cn/interview/12-Redis.html#_30-怎么处理热key)
...
![热key处理](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7160d2dde577716ef7f.png)
...
## [31.缓存预热怎么做呢？](http://www.susan.net.cn/interview/12-Redis.html#_31-缓存预热怎么做呢)
...
## [32.热点key重建？问题？解决？](http://www.susan.net.cn/interview/12-Redis.html#_32-热点key重建-问题-解决)
...
## [33.无底洞问题吗？如何解决？](http://www.susan.net.cn/interview/12-Redis.html#_33-无底洞问题吗-如何解决)
...
## [35.Redis的过期数据回收策略有哪些？](http://www.susan.net.cn/interview/12-Redis.html#_35-redis的过期数据回收策略有哪些)

Redis主要有2种过期数据回收策略：
![在这里插入图片描述](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7300d2dde57771728e2.png)
...
## [36.Redis有哪些内存溢出控制/内存淘汰策略？](http://www.susan.net.cn/interview/12-Redis.html#_36-redis有哪些内存溢出控制-内存淘汰策略)

Redis所用内存达到maxmemory上限时会触发相应的溢出控制策略，Redis支持六种策略：
![Redis六种内存溢出控制策略](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c73e0d2dde57771742e6.png)
...
## [37.Redis阻塞？怎么解决？](http://www.susan.net.cn/interview/12-Redis.html#_37-redis阻塞-怎么解决)

Redis发生阻塞，可以从以下几个方面排查：
![Redis阻塞排查](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7490d2dde5777175b07.png)
...
## [38.大key问题了解吗？](http://www.susan.net.cn/interview/12-Redis.html#_38-大key问题了解吗)
...
![大key处理](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7590d2dde5777177a5b.png)
...
## [39.Redis常见性能问题和解决方案？](http://www.susan.net.cn/interview/12-Redis.html#_39-redis常见性能问题和解决方案)
...
# Redis 看门狗（Watch Dog）机制：易懂 + 场景 + 核心逻辑

Redis 看门狗机制核心是 **Redis 分布式锁（Redlock/Redisson）中的 “自动续期” 逻辑**，解决 “持有锁的线程因超时 / 阻塞导致锁提前释放” 的问题。我用「通俗解释 + 业务场景 + 核心原理」讲清楚，你看完能直接理解并讲给别人。

## 一、先搞懂：为什么需要看门狗？

先看分布式锁的核心痛点：

分布式锁通常会设置**过期时间**（比如 10s），目的是防止线程死锁（比如线程拿到锁后宕机，锁永远不释放）。但问题来了：

- 如果线程拿到锁后，业务逻辑执行时间**超过了锁的过期时间**（比如处理 HIS 数据用了 15s），锁会被 Redis 自动释放；
- 此时其他线程会拿到锁，导致 “同一个资源被多个线程操作”（比如同时修改同一个患者的就诊记录），引发数据错乱。

**看门狗的核心作用**：

给持有锁的线程 “续命”—— 只要线程还在正常运行，就自动延长锁的过期时间，避免锁提前释放。

## 二、看门狗的核心逻辑（以 Redisson 为例，最常用）

Redisson 是 Redis 官方推荐的 Java 客户端，其分布式锁的看门狗机制是行业标准，逻辑如下：

### 1. 基础流程（一步一步讲）



是否线程1申请分布式锁Redis设置锁，默认过期时间30s（看门狗默认）线程1执行业务逻辑（比如调用HIS接口）线程1是否还在运行？看门狗定时任务（每10s）自动把锁过期时间重置为30s停止续期，锁到期自动释放其他线程可申请锁

### 2. 关键细节（必记）

- 默认参数

  - 锁的初始过期时间：30s（Redisson 默认，可配置）；
  - 续期频率：10s（是过期时间的 1/3，核心设计，避免续期不及时）；

- 触发条件

  只有线程持有锁且未执行完业务，看门狗才会续期；

  线程执行完业务后，会主动释放锁，看门狗停止续期；
...
- 实现方式：基于 Redis 的
  ...
  命令 + 客户端定时任务（Netty 的定时线程池），不是 Redis 服务端功能，是

  客户端实现。

## [41.Redis 如何实现延时队列?](http://www.susan.net.cn/interview/12-Redis.html#_41-redis-如何实现延时队列)

- **使用zset，利用排序实现**

可以使用 zset这个结构，用设置好的时间戳作为score进行排序，使用 zadd score1 value1 ....命令就可以一直往内存中生产消息。再利用 zrangebysocre 查询符合条件的所有待处理的任务，通过循环执行队列任务即可。
![zset实现延时队列](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7980d2dde5777180043.png)



## [43.Redis和Lua脚本的使用了解吗？](http://www.susan.net.cn/interview/12-Redis.html#_43-redis和lua脚本的使用了解吗)
...
## [44.Redis的管道了解吗？](http://www.susan.net.cn/interview/12-Redis.html#_44-redis的管道了解吗)
...
![Pipelining示意图`](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7bc0d2dde5777183a31.png)
...
## [45.Redis实现分布式锁了解吗？](http://www.susan.net.cn/interview/12-Redis.html#_45-redis实现分布式锁了解吗)
...
![setnx(set if not exists)](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7d10d2dde57771865ce.png)
...
![锁超时释放](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7dd0d2dde5777187c95.png)
...
![set原子指令](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7e90d2dde5777189cf3.png)
...
# [底层结构](http://www.susan.net.cn/interview/12-Redis.html#底层结构)

这一部分就比较深了，如果不是简历上写了精通Redis，应该不会怎么问。

## [46.说说Redis底层数据结构？](http://www.susan.net.cn/interview/12-Redis.html#_46-说说redis底层数据结构)
...
![redisObject对应的映射](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c7f70d2dde577718bc9e.png)
我们常用的数据类型和编码对应的映射关系：

![类型-编码-结构](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c8010d2dde577718cfc7.png)
...
![SDS](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c80e0d2dde577718ebed.png)
...
![链表linkedlist](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c8190d2dde5777190082.png)
...
1. **字典dict**：⽤于保存键值对的抽象数据结构。Redis使⽤hash表作为底层实现，一个哈希表里可以有多个哈希表节点，而每个哈希表节点就保存了字典里中的一个键值对。
   ...
   ![字典](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c8230d2dde57771915fe.png)
2. **跳跃表skiplist**：跳跃表是有序集合的底层实现之⼀，Redis中在实现有序集合键和集群节点的内部结构中都是⽤到了跳跃表。Redis跳跃表由zskiplist和zskiplistNode组成，zskiplist⽤于保存跳跃表信息（表头、表尾节点、⻓度等），zskiplistNode⽤于表示表跳跃节点，每个跳跃表节点的层⾼都是1-32的随机数，在同⼀个跳跃表中，多个节点可以包含相同的分值，但是每个节点的成员对象必须是唯⼀的，节点按照分值⼤⼩排序，如果分值相同，则按照成员对象的⼤⼩排序。
   ![跳跃表](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c8310d2dde5777193b40.png)
3. **整数集合intset**：⽤于保存整数值的集合抽象数据结构，不会出现重复元素，底层实现为数组。
   ![整数集合intset](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c83b0d2dde5777195420.png)
4. **压缩列表ziplist**：压缩列表是为节约内存⽽开发的顺序性数据结构，它可以包含任意多个节点，每个节点可以保存⼀个字节数组或者整数值。

![压缩列表组成](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c8440d2dde5777196a33.png)



## [49.跳跃表是如何实现的？原理？](http://www.susan.net.cn/interview/12-Redis.html#_49-跳跃表是如何实现的-原理)

PS:跳跃表是比较常问的一种结构。

跳跃表（skiplist）是一种有序数据结构，它通过在每个节点中维持多个指向其它节点的指针，从而达到快速访问节点的目的。
![跳跃表](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c8720d2dde577719cb87.png)
...
- **前进指针**
  ...
  ![通过前进指针遍历](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c8800d2dde577719effc.png)
...
- **跨度**
  ...
  ![计算节点的排位](https://web-1259428203.cos.ap-chengdu.myqcloud.com/6460c88d0d2dde57771a1108.png)
