...
![image-20250812204933056](JVM/image-20250812204933056.png)
...
synchronized 是 Java 内置的互斥锁，JDK 1.6 对其做了重大优化，引入「锁升级」机制（从无锁→偏向锁→轻量级锁→重量级锁），核心目标是 **在低并发下减少锁开销，高并发下保证线程安全**。

#### 1. 核心前提：对象头（Mark Word）

synchronized 锁的核心是「对象锁」，锁的状态会记录在对象的 **Mark Word**（对象头的核心部分）中，Mark Word 结构如下（64 位 JVM）：

 锁升级的本质：**根据并发情况，修改 Mark Word 中的锁状态标识，切换不同的锁实现方式**。

#### 2. 锁升级的完整流程（从低到高）

##### （1）无锁状态

- 初始状态，对象没有被任何线程锁定，Mark Word 记录对象哈希码、分代年龄等基础信息；
- 此时多个线程可以自由访问对象，无任何锁开销。

##### （2）偏向锁（低并发首选）

- **触发条件**：只有一个线程重复获取该对象锁，且无其他线程竞争；

- 核心逻辑

  ① 线程第一次获取锁时，JVM 在 Mark Word 中记录该线程的 ID，同时将锁状态改为「偏向锁」；

  ② 后续该线程再次获取锁时，只需检查 Mark Word 中的线程 ID 是否为自己，无需加锁 / 解锁操作，开销极低；

- **优势**：消除了「无竞争场景下的锁申请 / 释放开销」，是 JDK 1.6 默认的锁状态（可通过`-XX:-UseBiasedLocking`关闭）；

- **撤销条件**：当有第二个线程尝试竞争锁时，偏向锁会被撤销，升级为轻量级锁。

##### （3）轻量级锁（自旋锁，中低并发）

- **触发条件**：多个线程交替获取锁（无长时间阻塞），偏向锁撤销后升级；

- 核心逻辑

  ① 线程获取锁时，在自己的栈帧中创建「锁记录（Lock Record）」，并将 Mark Word 中的指针指向该锁记录；

  ② 其他线程竞争锁时，不会立即阻塞，而是通过「CAS 自旋」（循环尝试获取锁）等待锁释放；

- **优势**：避免了线程切换的内核态开销（自旋是用户态操作）；

- **自旋限制**：JDK 1.6 默认自旋次数为 10 次（或自适应自旋），自旋超过阈值仍未获取锁，则升级为重量级锁。

##### （4）重量级锁（互斥锁，高并发）

- **触发条件**：自旋次数超限，或多个线程同时竞争锁（出现长时间阻塞）；

- 核心逻辑

  ① 锁升级为重量级锁后，Mark Word 指向操作系统的「互斥量（Monitor）」；

  ② 竞争失败的线程会被挂起（进入阻塞状态），由操作系统调度，等待锁释放后唤醒；

- **劣势**：线程阻塞 / 唤醒需要切换到内核态，开销大；

- **优势**：适合高并发、长持有锁的场景，避免无意义的自旋消耗 CPU。
...
| :------: | :------------: | :----------------: | :---------------------: |
|   无锁   |     无竞争     |         无         |        无锁访问         |
|  偏向锁  | 单线程重复获取 | 极低（仅 ID 检查） |  Mark Word 记录线程 ID  |
| 轻量级锁 | 多线程交替获取 |   低（CAS 自旋）   |  栈帧锁记录 + CAS 自旋  |
| 重量级锁 | 多线程竞争阻塞 |  高（内核态切换）  | 操作系统 Monitor 互斥量 |

### synchronized vs ReentrantLock 核心区别

|      维度      |                         synchronized                         |                        ReentrantLock                         |
| :------------: | :----------------------------------------------------------: | :----------------------------------------------------------: |
|   **锁类型**   |                  内置锁（JVM 层面），隐式锁                  |        显式锁（API 层面，java.util.concurrent.locks）        |
|  **可重入性**  |                 支持（同一线程可重复获取锁）                 |                  支持（命名即 “可重入锁”）                   |
|   **锁释放**   |             自动释放（出代码块 / 方法自动释放）              |   必须手动释放（需在 finally 中调用 unlock ()，否则死锁）    |
|   **锁获取**   |                    只能被动等待，无法中断                    | 支持中断（lockInterruptibly ()）、超时获取（tryLock (long, TimeUnit)） |
|   **公平性**   |                  非公平锁（默认，无法修改）                  | 支持公平 / 非公平（构造函数指定：new ReentrantLock (true)）  |
|  **条件变量**  |              无，只能通过 Object 的 wait/notify              |   支持多个条件变量（newCondition ()），可精准唤醒指定线程    |
| **锁状态查询** |                   无法查询（JVM 内部维护）                   |      可查询（isLocked ()、isHeldByCurrentThread () 等）      |
...
其核心应用场景是‌**读操作远多于写操作的高并发场景**‌。它通过分离读锁和写锁，允许多个线程并发读取，但写入时保持独占，从而在保证数据一致性的前提下，显著提升系统吞吐量。

### 核心适用场景

1. ‌**缓存系统**‌：这是最经典的应用。缓存数据被频繁读取（如热点商品信息、用户会话），但更新（如缓存过期、手动刷新）频率很低。使用读写锁可以让成千上万的读请求并行执行，而写操作（更新缓存）则独占锁，避免了使用普通互斥锁时“读读互斥”的性能瓶颈。‌34
2. ‌**配置/状态管理**‌：系统中的全局配置（如功能开关、阈值参数）或服务状态（如是否可用）通常被大量线程并发读取，但修改操作（如运维调整）非常罕见。读写锁能高效支持海量读取，同时确保配置更新的原子性。‌34
3. ‌**统计与报表数据**‌：在线用户数、订单总量等统计信息被多个分析线程频繁读取，但更新（如由定时任务完成）频率较低。读写锁允许统计线程并发读取，避免了因写操作导致所有读操作被阻塞。‌3
4. ‌**高并发数据结构**‌：对于需要自定义的、读多写少的数据结构（如线程安全的字典、列表），若读操作（如 `get`）远多于写操作（如 `put`），使用读写锁替代 `synchronized` 或 `ReentrantLock` 能大幅减少读操作间的竞争。‌
...
![image-20250813205719260](JVM/image-20250813205719260.png)
...
![image-20250815213526879](JVM/image-20250815213526879.png)

>4种拒绝策略

![image-20250815214543022](JVM/image-20250815214543022.png)
...
![image-20250819213642072](JVM/image-20250819213642072.png)
...
![image-20250820212217915](JVM/image-20250820212217915.png)
...
![image-20250820214524037](JVM/image-20250820214524037.png)
