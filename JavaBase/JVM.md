...
类加载：1、类加载器（Bootstrap/Ext/App/Custom）根据类的全限定名从磁盘读取class文件的二进制字节流；
	  ...
	  4验证-准备（静态变量分配内存且初始化0值）-解析-初始化（静态变量赋默认值）-使用（堆中创建实列对象分配内存，将引用变量放在栈中设置对象头（Object Header）-执行初始化方法<init>()）
...
># 为什么 Java 要有那么多垃圾回收器？一个不行吗？
>
>**答案很简单：因为场景不一样，追求的目标不一样，一个回收器根本满足不了所有场景。**
>
>我给你拆开讲，你看完能直接教别人。
>
>------
>
># 1、先记住一句话：GC 有两个核心目标，天生互斥
>
>1. **吞吐量（Throughput）**
>
>   单位时间里，程序干活越多越好 → GC 少干点、慢点干都行。
>
>   → 适合：后台计算、大数据、批处理。
>
>   
...
>2. **低延迟（Low Latency）**
>
>   程序不能卡顿，一卡用户就崩 → GC 必须快、停顿极短。
>
>   → 适合：电商、支付、游戏、API 接口。
>
>   
>
>**这两个目标是矛盾的：想快，就不能一次干太多活；想干得多，就难免偶尔慢一下。**
>
>所以：
>
>**没有完美的 GC，只有适合场景的 GC。**
...
>- 给小单片机用 ZGC → 太复杂、带不动
>- 给大数据用 CMS → 吞吐量不够，跑得慢
>- 给游戏 / 金融用 Parallel GC → 一 Full GC 就卡顿，用户炸了
>
>**不同业务 = 不同要求 = 不同 GC。**
>
>就像：
